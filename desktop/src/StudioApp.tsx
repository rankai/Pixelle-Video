...
function SourceStep({
  ...
  return (
    <div>
          ...
          <div>
            ...
            {featureFlags.assetCenterV2 ? (
              <div className="inline-asset-picker">
                ...
                {session.state.brand_kit_id ? <button type="button" className="subtle" onClick={() => void patch({ brand_kit_id: "", brand_bgm_asset_id: "", bgm_asset_id: "" })}>清除</button> : null}
              </div>
            ) : (
              <select
      ...
      {featureFlags.assetCenterV2 ? (
        <AssetPickerDialog
          open={brandV2PickerOpen}
          kind="brand"
          selectedId={(session.state.brand_kit_id as string) || ""}
          context={{ session_id: session.session_id, step: "postproduction", purpose: "应用品牌", slot_id: "brand", allowed_kinds: ["brand"], required_capabilities: ["preview"], selection_mode: "single" }}
          onClose={() => setBrandV2PickerOpen(false)}
          onSelect={(item) => {
            const legacyBrand = brands.find((brand) => brand.brand_id === item.resource_id);
            const brandData = item.brand || {};
            void patch({
              brand_kit_id: item.resource_id,
              brand_bgm_asset_id: String(brandData.default_bgm_asset_id || ""),
              bgm_asset_id: "",
              bgm_path: legacyBrand?.default_bgm_path || session.state.bgm_path || "",
            });
            setBrandV2PickerOpen(false);
          }}
        />
      ) : null}
...
function VoiceStep({
  ...
  const [referenceVoiceV2Open, setReferenceVoiceV2Open] = useState(false);
  ...
  const selectedReferenceVoice = voices.find(
    (voice) => voice.reference_id === session.state.tts_ref_audio_id || voice.asset_path === session.state.tts_ref_audio_path,
  );
      ...
      <section className="modal large">
        ...
        <div className="storyboard-layout">
          ...
          <div className="storyboard-groups">
            <div className="storyboard-panel-title">
              <strong>覆盖组设置</strong>
              <small>每组可以保持数字人、选择视频/图片素材，或填写 AI 视频提示词。</small>
            </div>
            {groups.length ? (
              groups.map((group) => (
                  <GroupEditor
...
function GroupEditor({
  ...
  const selectedImage = images.find((item) => item.asset_id === group.image_asset_id);
  ...
  return (
    <section className="group-card">
      ...
      >
        ...
        {featureFlags.assetCenterV2 ? <option value="uploaded_image">图片素材覆盖</option> : null}
        ...
        <>
          ...
          {featureFlags.assetCenterV2 ? (
            <AssetPickerDialog
              ...
              context={{ session_id: sessionId, step: "visual_plan", purpose: "选择视频覆盖素材", slot_id: group.group_id, allowed_kinds: ["video"], required_capabilities: ["preview"], selection_mode: "single" }}
              onClose={() => setVideoPickerOpen(false)}
              onSelect={(item) => {
                update({
                  ...group,
                  video_asset_id: item.resource_id,
                  uploaded_video_path: item.file_url || "",
                });
      ...
      {group.visual_type === "uploaded_image" ? (
        <>
          <div className="section-title inline-section-title">
            <span>图片素材</span>
            <Space size={8}>
              <Button size="small" onClick={() => setImagePickerOpen(true)}>选择图片素材</Button>
              <Button size="small" onClick={() => openAssetTab("images")}>管理图片素材库</Button>
            </Space>
          </div>
          <div className="selected-video-summary">
            {selectedImage ? <><AssetImage src={selectedImage.file_url} alt={selectedImage.name} /><div><strong>{selectedImage.name}</strong><span>{selectedImage.filename}</span></div></> : <span>未选择图片素材。图片会按该覆盖组的时间段铺满画布。</span>}
          </div>
          {featureFlags.assetCenterV2 ? (
            <AssetPickerDialog
              open={imagePickerOpen}
              kind="image"
              selectedId={group.image_asset_id}
              context={{ session_id: sessionId, step: "visual_plan", purpose: "选择图片覆盖素材", slot_id: group.group_id, allowed_kinds: ["image"], required_capabilities: ["preview"], selection_mode: "single" }}
              onClose={() => setImagePickerOpen(false)}
              onSelect={(item) => {
                update({ ...group, image_asset_id: item.resource_id, uploaded_image_path: item.file_url || "" });
  ...
  return (
    <div className="modal-backdrop asset-modal-backdrop">
      <section className="modal confirm-delete-modal">
        ...
        {deleteError ? <Alert type="error" showIcon title={`删除失败：${deleteError}`} /> : null}
        <div className="modal-actions">
          <Button onClick={onClose}>取消</Button>
...
function AssetsView({
  ...
  const tabs: Array<{ key: AssetTab; label: string; icon: ReactNode }> = [
    { key: "videos", label: "视频素材", icon: <Video size={16} /> },
    { key: "images", label: "图片素材", icon: <ImageIcon size={16} /> },
  ...
  return (
    <section className="asset-center">
      ...
      {activeTab === "videos" ? <VideoLibrary items={assets.videos} reload={reload} assetCenterV2={assetCenterV2} /> : null}
      {activeTab === "images" ? <ImageLibrary items={assets.images} reload={reload} assetCenterV2={assetCenterV2} /> : null}
...
function ImageLibrary({
  ...
  const upload = assetCenterV2
    ? async (name: string, file: File) => mapV2ImageAsset(await uploadMediaAssetV2("image", name, file))
    : uploadImageAsset;
  const remove = assetCenterV2 ? archiveMediaAssetV2 : deleteImageAsset;
  return (
    <>
      <AssetLibraryShell
        title="图片素材"
        ...
        cards={
          <div className="asset-grid images">
            {items.map((item) => (
              <section key={item.asset_id} className="asset-card image-asset">
                ...
                <div className="asset-card-actions">
                  <button
                    type="button"
                    className="asset-action preview"
                    onClick={() => setPreview({ kind: "image", title: item.name, src: item.file_url })}
                  >
                    预览
                  </button>
                  <button
                    ...
                    onClick={() =>
                      setPendingDelete({
                        ...
                        confirmLabel: "确认删除图片素材",
                        confirm: async () => {
                          await remove(item.asset_id);
                          await reload();
                        },
                      })
                    }
            ...
            {!items.length ? (
              <div className="empty-state asset-library-empty">还没有图片素材。可先上传产品、门店、活动或客户案例图片。</div>
            ) : null}
          </div>
        }
      />
      <SimpleAssetModal
        open={addOpen}
        title="添加图片素材"
        description="上传产品、门店、活动或案例图片，后续可在画面规划和封面制作中复用。"
        assetNameLabel="图片素材名称"
        namePlaceholder="例如：夏季新品主图、门店外景、顾客案例"
        fileLabel="图片素材"
        ...
        submitLabel="保存到资产库"
        successNote="保存后可继续上传，或关闭弹窗返回图片素材库。"
        successMessage="已保存到图片素材库。"
      />
      <AssetPreviewModal preview={preview} onClose={() => setPreview(null)} />
...
function VideoLibrary({
  ...
  const [pendingDelete, setPendingDelete] = useState<PendingDelete | null>(null);
  const upload = assetCenterV2
    ? async (name: string, file: File) => mapV2VideoAsset(await uploadMediaAssetV2("video", name, file))
    : uploadVideoAsset;
  const remove = assetCenterV2 ? archiveMediaAssetV2 : deleteVideoAsset;
  return (
    <>
      <AssetLibraryShell
        title="视频素材库"
...
function ArtifactMediaPreview({
  ...
  useEffect(() => {
    ...
    artifactBlobUrl(sessionId, artifactKey)
      ...
      .catch((err) => {
        if (!disposed) setError(formatUiError(err));
      });
    return () => {
      disposed = true;
      if (current) URL.revokeObjectURL(current);
...
function v2Filename(item: AssetLibraryV2Item) {
  const path = item.revision.relative_path || "";
  return path.split("/").pop() || item.name;
}

function mapV2VideoAsset(item: AssetLibraryV2Item): VideoAsset {
  return {
    asset_id: item.asset_id,
    name: item.name,
    filename: v2Filename(item),
    created_at: item.created_at,
    duration: (item.revision.duration_ms || 0) / 1000,
    size: item.revision.bytes || 0,
    thumbnail_exists: Boolean(item.thumbnail_url),
    asset_path: item.file_url,
    file_url: item.file_url,
    thumbnail_url: item.thumbnail_url || "",
  };
}

function mapV2ImageAsset(item: AssetLibraryV2Item): ImageAsset {
  return {
    asset_id: item.asset_id,
    name: item.name,
    filename: v2Filename(item),
...
function mapV2BgmAsset(item: LibraryItemV2): BgmAsset {
  return {
...
function mapV2VoiceAsset(item: LibraryItemV2): VoiceAsset {
  const fileUrl = item.file_url || item.cover_url || "";
  return {
    reference_id: item.resource_id,
    name: item.name,
    filename: String(item.summary.filename || item.description || item.name),
    created_at: item.created_at,
    asset_path: fileUrl,
    file_url: fileUrl,
  };
...
function mapV2PortraitAsset(item: LibraryItemV2): PortraitAsset {
  const fileUrl = item.file_url || item.cover_url || "";
  return {
    portrait_id: item.resource_id,
    name: item.name,
    filename: String(item.summary.filename || item.description || item.name),
    created_at: item.created_at,
    media_type: item.summary.media_type === "video" ? "video" : "image",
    asset_path: fileUrl,
    file_url: fileUrl,
  };
}

function mapV2TemplateAsset(item: LibraryItemV2): IpTemplateAsset {
  const template = item.template || {};
  const coverContract = parseJsonRecord(template.cover_contract_json);
  const subtitleContract = parseJsonRecord(template.subtitle_contract_json);
  const previewUrl = String(template.preview_url || item.cover_url || "");
  return {
    template_id: String(template.template_id || item.resource_id),
    display_name: String(template.display_name || item.name),
    short_description: String(template.short_description || item.description || ""),
    full_description: String(template.full_description || item.description || ""),
    cover_template_path: previewUrl,
    preview_image_path: previewUrl,
    preview_url: previewUrl,
    subtitle_style: subtitleContract,
    render_subtitle_style: subtitleContract,
    render_canvas: {
      width: Number(coverContract.canvas_width || item.summary.canvas_width || 1080),
      height: Number(coverContract.canvas_height || item.summary.canvas_height || 1920),
    },
  };
}

function mapV2BrandKit(item: LibraryItemV2): BrandKit {
  const brand = item.brand || {};
  return {
    brand_id: item.resource_id,
    brand_name: String(brand.brand_name || item.name),
    created_at: item.created_at,
    logo_path: String(brand.logo_path || ""),
    primary_color: String(brand.primary_color || item.summary.primary_color || "#1f6feb"),
    secondary_color: String(brand.secondary_color || item.summary.secondary_color || "#0f766e"),
    font_family: String(brand.font_family || item.summary.font_family || ""),
    default_bgm_path: String(brand.default_bgm_path || ""),
    default_subtitle_style: String(brand.default_subtitle_style || ""),
    ending_card_text: String(brand.ending_card_text || item.description || ""),
    store_address: String(brand.store_address || ""),
    phone: String(brand.phone || ""),
    coupon_phrase: String(brand.coupon_phrase || ""),
  };
}
