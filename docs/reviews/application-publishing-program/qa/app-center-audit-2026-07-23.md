...
| 数字人口播视频 | AppRun、来源选择、数字人形象选择、TTS→数字人→后期真实媒体执行、恢复/取消/重试/人工接收 | 已接入 provider 生成闭环；真实 live smoke 受 RunningHub 余额阻塞；不含自动平台发布 | 4/5（独立页清晰，需余额恢复后完成一次真实成片验收） |
...
- `execute_provider` 已串接既有 `run_ip_broadcast_step` 的 TTS、数字人和后期步骤，设计上生成真实视频、封面和发布文案 Artifact；执行完成后停在 `needs_review`，不会自动触发平台发布。2026-07-24 单次真实 live smoke 中，Edge TTS 成功、RunningHub 音频/人物上传成功，但因企业版余额不足未创建数字人任务，视频/封面/发布文案 Artifact 尚未取得真实通过证据。
...
1. 在已配置真实 LLM、RunningHub/数字人服务和数字人资产的环境执行一次 live smoke，并记录生成耗时、失败码和人工接收证据。
...
- provider 边界测试验证了真实 TTS→数字人→后期调用链的 Artifact 产出和 `needs_review` 停手；2026-07-24 live smoke 真实调用了 Edge TTS 和 RunningHub 上传/任务创建，因余额不足停在 Provider 失败边界，尚未取得真实视频/封面/发布文案 Artifact；没有进行抖音/快手/小红书发布。
