...
| 数字人口播视频 | AppRun、来源选择、数字人形象选择、TTS→数字人→后期真实媒体执行、恢复/取消/重试/人工接收 | 已完成 provider 生成闭环；不含自动平台发布 | 4/5（独立页清晰，需真实 provider 配置验收） |
...
- `execute_provider` 已串接既有 `run_ip_broadcast_step` 的 TTS、数字人和后期步骤，生成真实视频、封面和发布文案 Artifact；执行完成后停在 `needs_review`，不会自动触发平台发布。
...
5. 仍需在已配置真实 LLM、RunningHub/数字人服务和至少一个数字人资产的环境执行一次 live smoke；平台自动点击仍按范围关闭。
...
- provider 边界测试验证了真实 TTS→数字人→后期调用链的 Artifact 产出和 `needs_review` 停手；本轮没有调用真实 LLM、RunningHub 或第三方平台，也没有进行抖音/快手/小红书发布。
