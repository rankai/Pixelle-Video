...
已批准例外：CR-PLATFORM-ORDER-001（2026-07-22）。当用户明确要求先完成平台适配时，可以暂停 PROGRAM-ROLLOUT 的外部证据闭环，进入 PLATFORM-EXPANSION 的“发布前适配”子阶段；该子阶段不得改变 PG-L 结论、不得打开默认 rollout、不得执行最终发布点击。平台适配收口后必须返回 PROGRAM-ROLLOUT 完成 Windows/签字/rollback 外部闭环，再进入平台正式灰度。
