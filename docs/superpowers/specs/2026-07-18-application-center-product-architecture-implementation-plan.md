...
```mermaid
flowchart TB
    UI["桌面端：应用中心 / 应用工作区 / 我的创作"]
    API["FastAPI：应用目录 / 项目 / 运行 / 产物 / 流转 API"]
    REG["Application Registry：版本化内置应用清单"]
    RUN["App Runner：统一校验、执行、进度、错误和重试"]
    EX1["Structured LLM Executor"]
    EX2["Document Render Executor"]
    EX3["Existing Workflow Adapter"]
    TASK["现有 TaskManager"]
    DATA["App Center SQLite Repository"]
    ASSET["企业资产库 V2"]
    IPB["现有 IpBroadcastWorkflow"]
...
| `video + publish_package` | 发布中心 | 视频、封面、标题、描述、标签 |
...
这些字段仅用于本地成本分析，不在首期展示给用户，也不直接扣套餐。未来可能的价值单位：文本运行次数、图文渲染页数、视频分钟数；现在不提前确定价格。

## 11. Luna 实施阶段与门禁

总原则：契约先行、阶段可回滚、旧口播不回归。任何阶段未通过对应 Gate，不得默认开启下一阶段功能。
...
2. ADR：应用注册表、项目/任务/产物边界、独立 SQLite、HashRouter 迁移；
...
- readiness 聚合 LLM、RunningHub、模板/品牌等现有配置；
- 不执行应用。
...
2. 新建项目 -> 生成文案 -> 人工编辑 -> 保存；
3. 文案 -> 标题 -> 选择主标题；
4. 文案/标题 -> 5 页图文 -> 替换一张图 -> 导出；
5. 文案 -> 数字人口播 -> 恢复已有 session -> 最终视频；
6. 图文/视频 -> 发布中心；
7. 生成中退出应用 -> 重启 -> 恢复状态；
8. 失败 -> 重试 -> 保留失败证据且成功产物不重复；
9. flag 全关 -> 旧工作台和口播完整可用。
...
```text
你正在 pixelle-video 仓库实施《Pixelle Video 应用中心产品规划与实施落地方案》的阶段 AC-X。

先阅读本方案、CLAUDE.md、当前 git status、相关 ADR/contract、当前实现和上一阶段 Gate 证据。工作区包含用户未提交改动，禁止覆盖、回退或顺手整理不在本阶段范围的文件。

只实施 AC-X 明确范围，不提前实现下一阶段。契约、迁移、失败语义、feature flag、测试和回滚证据属于交付内容，不是可选文档。新应用不得继续加入 StudioApp.tsx；路由只负责接线；业务逻辑不得写在 FastAPI router；所有 LLM 产品能力必须使用结构化 schema、确定性 validator 和 prompt version。
