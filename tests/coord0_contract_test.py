...
def reject_manifest_semantic(manifest: dict, registry: dict) -> None:
    ...
    for target in manifest["handoff_targets"]:
        if not set(manifest["produced_artifact_types"]).intersection(
            by_id[target]["accepted_artifact_types"]
        ):
...
def test_all_contract_json_is_valid_and_schemas_are_draft_2020_12():
    for path in (ROOT / "docs/contracts").rglob("*.json"):
        ...
        if (
            ...
            and value.get("$schema") == "https://json-schema.org/draft/2020-12/schema"
        ):
            Draft202012Validator.check_schema(value)
...
def test_manifest_and_artifact_fixtures_validate():
    ...
    for relative in ("artifact-copywriting.json", "artifact-publish-package-ref.json"):
        validate(
            "docs/contracts/app-center/artifact-contract.schema.json",
            read_json(f"docs/contracts/app-center/fixtures/{relative}"),
        )
    ...
    for manifest in manifests:
        for target in manifest["handoff_targets"]:
            assert set(manifest["produced_artifact_types"]).intersection(
                by_id[target]["accepted_artifact_types"]
            )
...
def test_app_input_output_state_error_and_flag_contracts_validate():
    ...
    for case in readiness:
        if not case["flag_enabled"]:
            expected = "disabled"
        elif not set(case["configured_capabilities"]) >= (
            {"llm"}
            if case["app_id"] != "builtin.digital-human-video"
            else {"llm", "runninghub", "digital_human"}
        ):
            expected = "not_ready"
        else:
            expected = "ready"
...
def test_artifact_semantic_invariants_are_executable():
    ...
    assert [item["revision"] for item in versions] == sorted(
        {item["revision"] for item in versions}
    )
...
def test_publish_package_run_and_step_fixtures_validate():
    ...
    artifact_source = read_json(
        "docs/contracts/publishing/fixtures/publish-package-artifact-source.json"
    )["source"]
    ...
    validate(
        ...
        read_json("docs/contracts/publishing/fixtures/publish-account-douyin-pilot.json"),
    )
    ...
    validate(
        "docs/contracts/publishing/publish-step-result.schema.json",
        read_json("docs/contracts/publishing/fixtures/publish-step-result-waiting-human.json"),
    )
...
def test_profile_discovery_is_read_only_and_redacted():
    report = read_json(
        "docs/contracts/publishing/fixtures/profile-discovery-report-2026-07-19.json"
    )
    ...
    assert all(
        candidate["candidate_ref"].startswith("relative:") for candidate in report["candidates"]
    )
...
def test_current_publishing_baseline_has_nine_explicit_external_rows():
    baseline = read_json(
        "docs/contracts/publishing/fixtures/publish-v2-current-baseline-2026-07-19.json"
    )
    ...
    for row in baseline["tasks"]:
        ...
        assert (row["click_count"], row["automation_ms"], row["human_wait_ms"]) == (
            metric["click_count"],
            metric["automation_ms"],
            metric["human_wait_ms"],
        )
...
def test_minimal_douyin_dom_fixtures_are_present_redacted_and_cover_gate_states():
    ...
    for item in fixtures:
        ...
        html = path.read_text()
        assert "sha256:" + hashlib.sha256(path.read_bytes()).hexdigest() == item["sha256"], item[
            "fixture_id"
        ]
...
def test_media_fixture_manifest_covers_valid_and_invalid_inputs():
    ...
    with tempfile.TemporaryDirectory(prefix="pixelle-coord0-media-") as directory:
        result = subprocess.run(
            [
                sys.executable,
                "docs/contracts/publishing/generate_media_fixtures.py",
                "--output-dir",
                directory,
            ],
            cwd=ROOT,
        ...
        assert all(
            item["sha256"].startswith("sha256:") and item["bytes"] >= 0 for item in generated
        )
...
def test_final_action_guard_and_model_redaction_fixtures_are_safe():
    ...
    assert all(
        case["expected"] == "reject_model_override" for case in redaction["request_override_cases"]
    )
    assert all(
        not any(
            secret in json.dumps(redaction["response_metadata"])
            for secret in redaction["forbidden_persisted_values"]
        )
        for _ in [0]
    )
...
def test_guard_and_v1_rollback_local_smoke_evidence_is_explicitly_bounded():
    evidence = read_json(
        "docs/reviews/application-publishing-program/qa/COORD-0-guard-rollback-local-smoke-2026-07-19.json"
    )
    ...
    for action_id in ("publish", "confirm_publish", "unknown"):
        assert decisions[action_id] == {
            "action_id": action_id,
            "decision": "deny",
            "error_code": "FINAL_ACTION_BLOCKED",
        }
...
def test_sqlite_migrations_are_idempotent_and_constraints_hold():
    ...
    connection.execute(
        "CREATE TABLE publish_packages (id TEXT PRIMARY KEY, session_id TEXT NOT NULL)"
    )
    for relative in (
        "docs/contracts/app-center/app-center-v1.sql",
        "docs/contracts/publishing/publishing-v2.sql",
    ):
    ...
    assert {
        ...
        "app_events",
        "artifact_handoffs",
        "publishing_schema_migrations",
        "publish_accounts",
        "publish_platform_release",
        "publish_packages_v2",
        "publish_runs_v2",
        "publish_step_results",
        "publish_run_step_attempts",
        "publish_events",
    }.issubset(tables)
    assert "publish_packages" in tables
    connection.execute(
        "INSERT INTO publish_packages(id, session_id) VALUES ('legacy_1', 'session_1')"
    )
    assert connection.execute("SELECT count(*) FROM publish_packages").fetchone()[0] == 1
    assert (
        connection.execute(
            "SELECT schema_version FROM app_schema_migrations WHERE migration_id='app-center-v1'"
        ).fetchone()[0]
        == 1
    )
    assert (
        connection.execute(
            "SELECT schema_version FROM publishing_schema_migrations WHERE migration_id='publishing-v2'"
        ).fetchone()[0]
        == 2
    )
    release_states = dict(
        connection.execute(
            "SELECT platform, release_state FROM publish_platform_release"
        ).fetchall()
    )
    ...
    with pytest.raises(sqlite3.IntegrityError):
        connection.execute(
            "INSERT INTO app_schema_migrations(migration_id, schema_version, applied_at) VALUES ('future', 99, 'now')"
        )
    with pytest.raises(sqlite3.IntegrityError):
        connection.execute(
            "INSERT INTO publishing_schema_migrations(migration_id, schema_version, applied_at) VALUES ('future', 99, 'now')"
        )
    connection.execute(
        "INSERT INTO publish_accounts(account_id, platform, display_name, profile_ref, created_at) VALUES ('acct_1','douyin','pilot','profile_1','now')"
    )
    connection.execute(
        "INSERT INTO publish_packages_v2(package_id, project_id, source_kind, source_artifact_ids_json, source_artifact_version_ids_json, source_revision, artifact_refs_json, package_fingerprint, created_at) VALUES ('pkg_1','p','artifact_versions','[\"a\"]','[\"v\"]','r1','[]','sha256:p','now')"
    )
    connection.execute(
        "INSERT INTO publish_runs_v2(run_id, package_id, account_id, platform, state, idempotency_key, created_at, updated_at) VALUES ('run_1','pkg_1','acct_1','douyin','queued','idem_1','now','now')"
    )
    connection.execute(
        "UPDATE publish_runs_v2 SET state='running', state_version=2 WHERE run_id='run_1'"
    )
    connection.execute(
        "UPDATE publish_runs_v2 SET state='waiting_for_human', state_version=3 WHERE run_id='run_1'"
    )
    with pytest.raises(sqlite3.IntegrityError):
        connection.execute(
            "INSERT INTO publish_packages_v2(package_id, project_id, source_kind, source_artifact_ids_json, source_artifact_version_ids_json, source_revision, artifact_refs_json, package_fingerprint, created_at) VALUES ('pkg_bad','p','artifact_versions','[]','[]','r1','[]','sha256:bad','now')"
        )
    with pytest.raises(sqlite3.IntegrityError):
        connection.execute("UPDATE publish_runs_v2 SET human_confirmed=1 WHERE run_id='run_1'")
    connection.execute(
        "INSERT INTO publish_step_results(step_result_id, run_id, step, state, evidence_kind, evidence_redacted, created_at) VALUES ('step_1','run_1','await_human_publish','waiting_for_human','none',1,'now')"
    )
    with pytest.raises(sqlite3.IntegrityError):
        connection.execute(
            "INSERT INTO publish_step_results(step_result_id, run_id, step, state, evidence_kind, evidence_redacted, created_at) VALUES ('step_bad','run_1','await_human_publish','waiting_for_human','log_ref',0,'now')"
        )
    connection.commit()
    try:
    ...
    assert (
        connection.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='injected_migration_table'"
        ).fetchone()
        is None
    )
...
def test_coordination_and_plan_convergence_is_explicit():
    ...
    assert re.search(
        r"^current_stage: (APP-TEXT|PUB-ACCOUNT|PUB-CORE|PUB-DOUYIN|APP-CAROUSEL|APP-IPB|PUB-INTEGRATION|E2E-DOUYIN|PROGRAM-ROLLOUT|PLATFORM-EXPANSION|APP-WORKBENCH|BRAND-PROJECT|APP-RESULT-HISTORY-[0-4])$",
        progress,
        re.MULTILINE,
    )
    assert re.search(
        r"^current_stage_status: (in_progress|waiting_user|implementation_in_progress|delivery_in_progress|entry_in_progress|paused_external)$",
        progress,
        re.MULTILINE,
    )
    assert re.search(
        r"^gate_status: (PG-D/entry_passed_with_boundary|PUB-ACCOUNT/entry_passed_with_boundary|PUB-CORE/entry_in_progress|PUB-CORE/entry_passed_with_boundary|PUB-DOUYIN/entry_in_progress|PUB-DOUYIN/entry_passed_with_boundary|PUB-DOUYIN/implementation_pass_with_boundary|PUB-DOUYIN/pg_g_in_progress|APP-CAROUSEL/entry_in_progress|APP-CAROUSEL/entry_passed_with_boundary|APP-CAROUSEL/implementation_in_progress|APP-CAROUSEL/implementation_pass_with_boundary|APP-CAROUSEL/PG-H_in_progress|APP-CAROUSEL/PG-H_passed_with_boundary|APP-IPB/entry_in_progress|APP-IPB/implementation_in_progress|APP-IPB/implementation_pass_with_boundary|PUB-INTEGRATION/pg-j-closure-entry_passed_with_boundary|PUB-INTEGRATION/pg-j-closure-implementation_pass_with_boundary_pending_review|E2E-DOUYIN/pub-5-entry_in_progress|E2E-DOUYIN/pub-5_entry_passed_with_boundary|E2E-DOUYIN/pub-5-entry_passed_with_boundary|PROGRAM-ROLLOUT/entry_passed_with_boundary|PROGRAM-ROLLOUT/implementation_in_progress|PROGRAM-ROLLOUT/implementation_pass_with_boundary|PROGRAM-ROLLOUT/PG-L_waiting_user|PLATFORM-EXPANSION/entry_in_progress|PLATFORM-EXPANSION/PG-M-PILOT-RELEASE_in_progress|PLATFORM-EXPANSION/PG-M-PILOT-RELEASE_passed_with_boundary|PG-AW-E_implementation_in_progress|PG-AW-E_passed_with_boundary|PG-AW-F_entry_in_progress|PG-AW-G_passed_with_boundary|PG-AW-H_entry_in_progress|PG-AW-H_implementation_in_progress|PG-AW-H_review_in_progress|PG-AW-H_passed_with_boundary|PG-BP-A_entry_in_progress|PG-BP-B_implementation_in_progress|PG-BP-B_implementation_review_pending|PG-BP-C_implementation_in_progress|PG-BP-C_implementation_review_pending|PG-BP-D_implementation_in_progress|PG-BP-D_implementation_review_pending|PG-BP-E_implementation_in_progress|PG-BP-E_implementation_review_pending|PG-BP-E_remediation_review_pending|PG-BP-F_implementation_in_progress|PG-BP-F_implementation_review_pending|PG-ARH-A_entry_in_progress|PG-ARH-A_passed_with_boundary|PG-ARH-B_implementation_in_progress|PG-ARH-B_passed_with_boundary|PG-ARH-C_implementation_in_progress|PG-ARH-C_passed_with_boundary|PG-ARH-D_implementation_in_progress|PG-ARH-D_passed_with_boundary|PG-ARH-E_review_in_progress|PG-ARH-E_passed_with_boundary)$",
        progress,
        re.MULTILINE,
    )
    assert re.search(
        r"\| 0 \| COORD-0 \| AC-0 \+ PUB-0 \| `completed` \| PG-A \| `passed` \|", progress
    )
    assert re.search(
        r"\| 1 \| APP-SHELL \| AC-1 \| `completed` \| PG-B \| `passed_with_boundary` \|", progress
    )
    assert re.search(
        r"\| 2 \| APP-CORE \| AC-2 \| `completed` \| PG-C \| `passed_with_boundary` \|", progress
    )
    assert re.search(
        r"\| 3 \| APP-TEXT \| AC-3 \| `completed` \| PG-D \| `passed_with_boundary` \|", progress
    )
    assert re.search(
        r"\| 4 \| PUB-ACCOUNT \| PUB-1 \| `completed` \| PG-E \| `passed_with_boundary` \|",
        progress,
    )
    assert re.search(
        r"\| 5 \| PUB-CORE \| PUB-2 \| `completed` \| PG-F \| `passed_with_boundary` \|", progress
    )
    assert re.search(
        r"\| 6 \| PUB-DOUYIN \| PUB-3 \| `completed` \| PG-G \| `passed_with_boundary` \|", progress
    )
    assert re.search(
        r"\| 7 \| APP-CAROUSEL \| AC-4 \| `completed` \| PG-H \| `passed_with_boundary` \|",
        progress,
    )
    assert re.search(
        r"\| 8 \| APP-IPB \| AC-5 \| `completed` \| PG-I \| `passed_with_boundary` \|", progress
    )
    assert re.search(
        r"\| 9 \| PUB-INTEGRATION \| PUB-4 \| `completed` \| PG-J \| `passed_with_boundary` \|",
        progress,
    )
    assert re.search(
        r"\| 10 \| E2E-DOUYIN \| PUB-5 \| `completed_with_boundary` \| PG-K \| `passed_with_boundary` \|",
        progress,
    )
    assert re.search(
        r"\| 11 \| PROGRAM-ROLLOUT \| AC-6 \+ PUB-7D \| `delivery_in_progress` \| PG-L \| `implementation_pass_with_boundary` \|",
        progress,
    )
    assert re.search(
        r"\| 12 \| PLATFORM-EXPANSION \| PUB-6 \+ 分平台 PUB-7 \| `completed_with_boundary` \| PG-M-PILOT-RELEASE \| `passed_with_boundary` \|",
        progress,
    )
    app_plan = (
        ROOT
    ...
    publish_plan = (
        ROOT / "docs/reviews/2026-07-18-desktop-auto-publishing-refactor-implementation-plan.md"
    ).read_text()
