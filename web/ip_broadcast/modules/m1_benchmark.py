...
def _render_tab_brain(pixelle_video):
    ...
    if st.button("生成口播文案", key="ipb_brain_generate_btn", use_container_width=True, type="primary"):
        ...
        try:
            ...
            _finish_step1_task("success", "IP文案生成完成")
...
def _learn_from_profile(pixelle_video, profile_url: str):
    ...
    try:
        ...
        if not urls:
            ...
            _finish_step1_task("warning", "未抓取到视频链接，请手动粘贴最近 5 条视频链接继续学习。")
        ...
        st.session_state.ipb_ip_show_manual_fallback = True
        _finish_step1_task("warning", str(e))
        st.warning(str(e))
    except Exception as e:
        st.session_state.ipb_ip_show_manual_fallback = True
...
def _learn_from_video_inputs(pixelle_video, video_inputs: list[str], label: str):
    ...
    try:
        ...
        if not scripts:
            _finish_step1_task("warning", "未能从这些视频中提取到可用口播文案，请检查链接或手动粘贴脚本。")
        ...
        _finish_step1_task("success", f"已学习 {len(scripts)} 条视频文案，生成 {len(result.topics)} 个选题")
    except Exception as e:
    ...
    if topics:
        ...
        if st.button("为此选题生成文案", key="ipb_ip_script_btn", use_container_width=True):
            ...
            try:
                ...
                _finish_step1_task("success", "文案生成完成")
            except Exception as e:
                _finish_step1_task("error", str(e))
                st.error(str(e))
                logger.exception(e)
...
def _clean_and_set_source(pixelle_video, raw_text: str, label: str):
    ...
    try:
        ...
        _finish_step1_task("success", f"文案已准备好（{len(cleaned)} 字符）")
    except Exception as e:
        _finish_step1_task("error", str(e))
        st.error(str(e))
        logger.exception(e)
